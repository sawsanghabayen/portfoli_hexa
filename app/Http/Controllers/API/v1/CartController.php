...
//            return $options_ids;
        ...
        if ($test_cart) {
            if (@$test_cart->meal->user_id != $meal->user_id) {
                $message = __('api.youCanNotAddFromAnotherRestaurant');
                return response()->json(['status' => false, 'code' => 203, 'message' => $message]);
            }
        }

        $meal_id = $request->meal_id;
        $check = Cart::where('meal_id', $meal_id)->where(function ($q) use ($request) {
            $q->where('fcm_token',  $request->header('fcmToken'))->orWhere('user_id', auth('api')->id());
        })->first();
        if ($check) {
            $message = __('api.added_before');
            return response()->json(['status' => false, 'code' => 403, 'message' => $message]);
        }
        $item = new Cart();
...
//        }
    ...
    public function removeFromCart(Request $request)
    {
        ...
        $item = Cart::where('meal_id', $request->meal_id)->where(function ($q) use ($request) {
            $q->where('fcm_token', $request->header('fcmToken'))->orWhere('user_id', auth('api')->id());
        })->first();
        if ($item) {
            $item->delete();
    ...
    {
        $item = Order::where('id', $id)->with(['provider', 'meals'])->first();
        if (!$item) {
        ...
        return response()->json(['status' => true, 'code' => 200,
            'message' => __('api.ok'), 'item' => $item->makeHidden('meals')]);
    }
